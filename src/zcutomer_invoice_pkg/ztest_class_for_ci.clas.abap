CLASS ztest_class_for_ci DEFINITION
  PUBLIC
...
CLASS ZTEST_CLASS_FOR_CI IMPLEMENTATION.
...
*out->write( it_item ).
    ...
    WHERE a~AccountingDocument = '1800000017' AND a~AmountInCompanyCodeCurrency < -1 AND
     a~transactiontypedetermination  = 'JOC' AND a~CompanyCode = 'BNAL' AND a~FiscalYear = '2025'
...
*   out->write( it_cgst_amt ).
    ...
    a~transactiontypedetermination
    WHERE a~AccountingDocument = '1800000017' AND a~AmountInCompanyCodeCurrency < -1 AND
     a~transactiontypedetermination  = 'JOS' AND a~CompanyCode = 'BNAL' AND a~FiscalYear = '2025'
...
srno  = srno + 1.
      ...
      AccountingDocument = wa_line-AccountingDocument  CompanyCode = wa_line-CompanyCode  FiscalYear = wa_line-FiscalYear.

      DATA(lv_cgstAmt) =
|<cgstAmt>{ wa_camt-AmountInCompanyCodeCurrency }</cgstAmt>|.
out->write( it_cgst_amt ).
      READ TABLE it_sgst_amt INTO DATA(wa_samt) WITH KEY AccountingDocument = wa_line-AccountingDocument  CompanyCode = wa_line-CompanyCode  FiscalYear = wa_line-FiscalYear.
      DATA(lv_sgstAmt) =
              |<sgstAmt>{ wa_samt-AmountInCompanyCodeCurrency }</sgstAmt>|.


      READ TABLE it_igst_amt INTO DATA(wa_iamt) WITH KEY AccountingDocument = wa_line-AccountingDocument  CompanyCode = wa_line-CompanyCode  FiscalYear = wa_line-FiscalYear .
      DATA(lv_igstAmt) =
